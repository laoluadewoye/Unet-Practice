"""
Module for defining 3-dimensional UNET modules.
"""
import torch
import math
import copy
...
class DoubleConvThree(nn.Module):
    ...
    def forward(self, batch, time_embed=None):
        ...
        if self.need_time:
            ...
            # Expands the shape to (batch, out_channels, 1, 1, 1)
...
class SpatialAttentionThree(nn.Module):
    def __init__(self, dec_skip_channels, inter_channels, use_pool=False):
        ...
        self.decoder_conv = nn.Sequential(
            nn.Conv3d(dec_skip_channels, inter_channels, kernel_size=1),
...
class UpSampleThree(nn.Module):
    def __init__(self, in_channels, out_channels, up_drop_perc=0.3, use_attention=False, attn_pool=False,
        ...
        if self.need_attention:
            self.attention = SpatialAttentionThree(in_channels // 2, out_channels // 2, use_pool=attn_pool)
        else:
            self.attention = None
